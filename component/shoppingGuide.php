<?php ?>
<div class="container mt-5">
    <div class="row">
        <div class="col-12 kakomi">
            <div class="row title-header border-bottom-lightgrey py-2">
                <h1 class="title px-3">ショッピングガイド</h1>
            </div>
            <div class="row">
                <div class="col-12 col-md-3 py-2 border-lightgrey">
                    <h2 class="title col-12 mb-2 font-weight-bold">電話でのご注文</h2>
                    <div class="text-center">
                        <img class="lazy" data-src="img/phone.jpg" style="width: auto; max-height:100px;">
                    </div>
                    <div class="mt-2 text-right">
                        <button class="btn btn-info font-size-medium">
                            もっと詳しく
                        </button>
                    </div>
                </div>
                <div class="col-12 col-md-3 py-2 border-lightgrey">
                    <h2 class="title col-12 mb-2 font-weight-bold">モバイルでのご注文</h2>
                    <div class="text-center">
                        <img class="lazy" data-src="img/qr.gif" style="width: auto; max-height:100px;">
                        <p class="font-size-medium text-left">スマホでwww.3838.comにアクセス</p>
                    </div>
                    <div class="mt-2 text-right">
                        <button class="btn btn-info font-size-medium">
                            もっと詳しく
                        </button>
                    </div>
                </div>
                <div class="col-12 col-md-3 py-2 border-lightgrey">
                    <h2 class="title col-12 mb-2 font-weight-bold">送料について</h2>
                    <div class="text-center">
                        <img class="lazy" data-src="img/ship.gif" style="max-width: 100%; max-height:100px;">
                    </div>
                    <p class="font-size-medium">※送料が必要な場合、全国一律６００円。</p>
                    <div class="mt-2 text-right">
                        <button class="btn btn-info font-size-medium">
                            もっと詳しく
                        </button>
                    </div>
                </div>
                <div class="col-12 col-md-3 py-2 border-lightgrey">
                    <h2 class="title col-12 mb-2 font-weight-bold">お支払いについて</h2>
                    <div class="text-center">
                        <img class="lazy" data-src="img/pay.gif" style="width: auto; max-height:100px;">
                    </div>
                    <div class="mt-2 text-right">
                        <button class="btn btn-info font-size-medium">
                            もっと詳しく
                        </button>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>